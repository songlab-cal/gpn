...
modernization review does not authorize them. Merge and final-release actions
become approval-ready only after the final review packet names the single PR that
binds the exact head and tree. `depends_on`
...
The `0.9.0` review packet lives under `release/0.9.0/`. The pull-request body
binds review to the final head commit and tree because a Git object cannot embed
its own identity. Nothing in this directory grants authorization to merge,
publish, change repository settings, or write to Hugging Face.
