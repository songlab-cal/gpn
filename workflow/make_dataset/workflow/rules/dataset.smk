...
rule make_dataset_assembly:
    ...
    output:
        temp(
            expand(
                "results/dataset_assembly/{{assembly}}/{split}.parquet", split=splits
            )
        ),
    threads: 2
    run:
        ...
        chrom_split[np.isin(chroms, config["whitelist_validation_chroms"])] = (
            "validation"
        )
...
rule merge_datasets:
    input:
        expand(
            "results/dataset_assembly/{assembly}/{{split}}.parquet",
