...
class ModelforVEPModel(torch.nn.Module):
    ...
    def get_log_odds(self, input_ids, pos):
        other_pos = torch.ones(input_ids.shape[1], dtype=torch.bool)
        other_pos[pos] = False
        logits = self.model(input_ids=input_ids).logits[:, other_pos]
        logits = logits[:, :, -4:]
    ...
    def forward(
        ...
        fwd = self.get_score(input_ids_ref_fwd, input_ids_alt_fwd, pos_fwd)
        rev = self.get_score(input_ids_ref_rev, input_ids_alt_rev, pos_rev)
        return (fwd + rev) / 2
