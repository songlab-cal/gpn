...
pred.loc[:, nucleotides] = (
    softmax(pd.read_parquet(snakemake.input[1])[nucleotides].values, axis=1) - 1 / 4
)  # centering
...
pos_patterns, neg_patterns = modiscolite.tfmodisco.TFMoDISco(
    ...
    one_hot=sequences,
    max_seqlets_per_metacluster=1_000_000,  # default is 2000
    sliding_window_size=20,
    flank_size=5,
    target_seqlet_fdr=0.05,
    n_leiden_runs=5,  # default is 2
    verbose=True,
)
