...
def stratified_bootstrap_se(
    y_true, y_score, n_bootstraps=1000, seed=42, return_mean=False
):
    """
    ...
    if return_mean:
        return (
            np.std(boot_scores_auroc, ddof=1),
            np.std(boot_scores_auprc, ddof=1),
            np.mean(boot_scores_auroc),
            np.mean(boot_scores_auprc),
        )
    else:
...
def barplot(
    df, metric, palette, figsize=(2, 2.25), pos_prop=None, title=None, save_path=None
):
    ...
    metric_values = df[metric].apply(
        lambda x: x[0] if isinstance(x, (tuple, list)) else x
    )
    se_values = df[f"{metric}_se"].apply(
        lambda x: x[0] if isinstance(x, (tuple, list)) else x
    )
    ...
    ax.xaxis.set_minor_locator(
        AutoMinorLocator(2)
    )  # Increase divisor to make minor ticks sparser
...
def barplot_with_numbers(
    ...
    for group, ax in zip(groups, axes.flat):
        ...
        for bar, model in zip(g.patches, df_g.Model):
            ...
            g.text(
                max(bar.get_width(), baseline)
                + 0.01,  # X position, here at the end of the bar
                bar.get_y()
...
def barplot_vertical_aligned(
    ...
    for ax, group in zip(axes, groups):
        ...
        n_pos, n_neg = df_g.n_pos.iloc[0], df_g.n_neg.iloc[0]
        baseline = (n_pos / (n_pos + n_neg)) if metric == "AUPRC" else const_baseline

        # b) draw vertical bars
        sns.barplot(
            data=df_g, x="Model", y=metric, order=model_order, palette=palette, ax=ax
        )
