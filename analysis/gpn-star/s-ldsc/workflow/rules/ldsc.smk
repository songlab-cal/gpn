...
rule quantile_score_consequence_group:
    input:
        "results/variant_scores/{model}.parquet",
        "results/maf/merged.parquet",
        "results/variants/merged.annot_with_cre_v2.parquet",
    output:
        "results/variant_scores/quantile_group_consequence_{group}/{model}/{q}.parquet",
    run:
        q = float(wildcards.q)
        group_consequences = config["consequence_groups"][wildcards.group]
        V = pd.read_parquet(input[0])
        MAF = pd.read_parquet(input[1])
        consequences = pd.read_parquet(input[2], columns=["consequence"])
        assert len(V) == len(MAF) == len(consequences)
        ...
        V["common"] = V.MAF > 0.05
        # ensure variants with other consequences are not picked
        V.loc[~V.consequence.isin(group_consequences), "score"] = V.score.min() - 1
        print(V)
        n = int(V.common.sum() * q)
        V2 = V.copy()
        V2["idx_V1"] = np.arange(len(V2))
...
rule score_group_consequence_annotation:
    input:
        "results/variants/merged.annot_with_cre_v2.parquet",
    output:
        "results/variant_scores/group_consequence_{group}.parquet",
    run:
        V = pd.read_parquet(input[0], columns=["consequence"])
        group_consequences = config["consequence_groups"][wildcards.group]
        V["score"] = V.consequence.isin(group_consequences).astype(int)
        V[["score"]].to_parquet(output[0], index=False)
